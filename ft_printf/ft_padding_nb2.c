...
int		print_spaces_padding_before(t_options *option, int len, long long nb, int after)
{
	...
	len = option->pad_deux > len ? (option->pad_deux - get_sign_option) : len; /* ok mais attention à bien diminuer le pad-max du signe car celui ci n est pas compté dans len */
	//printf("len %d\n", len);
	...
	i = -1;
	ret = 0;
	// printf("i < option->pad_un = %d |-| len = %d |-| get_sign_option = %i |-| un_space_a_til_ete_print = %i\n", option->pad_un, len, get_sign_option, un_space_a_til_ete_print);
	while (++i < option->pad_un - len - get_sign_option /* normal */ - un_space_a_til_ete_print)
	{
		// printf("get_sign_option = %i ||", get_sign_option);
		if (option->pad_un && i + 1 == (option->pad_un - len - get_sign_option - un_space_a_til_ete_print))
		{
			...
			&& option->pad_deux) /* si pad max == 0 alors on print ' ' car ça veut dire qu il n y  aura pas de 0 a mettre */
			{
	...
	if (sign_is_print) // si on a print le sign en before alors on augmente le pad_un de 1 car on printera pas le sign dans print_zeros_padding
	{
		option->sign_is_print = 1;
		// option->pad_deux += 1; /* ligne a supprimer sauf si erreur d ici fin du projet */
	}
	if (!after && !option->pad_deux && !sign_is_print)
	{
		// write(1, "here\n", 5);
		ret += print_sign(option, nb);
	}
...
int		print_spaces_padding_after(t_options *option, int len, long long nb, int after) // sert a print les spaces padding en toute circonstances
{
	...
	len = option->pad_deux > len ? option->pad_deux : len;
	i = -1;
	ret = 0;
	...
	while (++i < option->pad_un - (len) - nb_have_a_sign(option, nb) - un_space_a_til_ete_print)
	{
		// write(1, "here\n", 5);
		ret += ft_putchar_ret(' ');
	}
...
int		print_zeros_padding_before(t_options *option, int len, long long nb) // sert a print les 0 padding en toute circonstances
{
	...
	// printf("len =%d ", len);
	//////////////////////////////////////////////////////////////////////////////
	if (option->pad_deux) // 5.1
	{
		if (option->pad_un) // 5.1.1
		{
			if (option->left_zeros) // final 5.1.1.1
			{
				if (option->pad_deux > len)
				{
					printf("i < %d - %d - %d - %d + %d\n", option->pad_deux, len, nb_have_a_sign(option, nb), (option->space && !nb_have_a_sign(option, nb) && option->pad_un > option->pad_deux ? 1 : 0), (option->pad_deux - len));
					while (++i < option->pad_deux - len - nb_have_a_sign(option, nb) /* normal */
					- (option->space && !nb_have_a_sign(option, nb) /* si il y a un space pour un nb positif */
					&& option->pad_un >= option->pad_deux ? 1 : 0) /* je sais plus pourquoi */
				  + (option->pad_deux - len)) /* il faut mettre des 0 entre les 2 */
					{
						//printf("ahere");
						ret += ft_putchar_ret('0');
					}
				...
				{
					// printf("%i < %d - %d + %d - %d\n", i, option->pad_deux, (len - option->sign_is_print), (nb_have_a_sign(option, nb) && !option->left_justify), (option->space && option->pad_un > option->pad_deux ? 1 : 0));
					while (++i < option->pad_deux - len - nb_have_a_sign(option, nb)
					- (option->space && !nb_have_a_sign(option, nb)
					&& option->pad_un > option->pad_deux ? 1 : 0))
					{
						printf("len =%d ", len);
						ret += ft_putchar_ret('0');
					}
			...
			{
				if (option->pad_deux >= option->pad_un) /* pourquoi ? */
				{
				...
				{
					// printf("i < option->pad_deux = %i |-| len = %d |-| nb_have_a_sign(option, nb) = %i", option->pad_deux, len, nb_have_a_sign(option, nb));
					while (++i < option->pad_deux - len - nb_have_a_sign(option, nb) /* normal */
		...
		else if (!option->pad_un)  // 5.1.2
		{
			if (option->left_zeros)   // final 5.1.2.1
			{
				// printf("padmax - %d + %d - %d\n", len, (nb_have_a_sign(option, nb) && !option->left_justify), (option->space && option->pad_un > option->pad_deux ? 1 : 0));
				// probablement a changer :
				while (++i < option->pad_deux - len + (nb_have_a_sign(option, nb) && !option->left_justify) - (option->space && option->pad_un > option->pad_deux ? 1 : 0))
				{
					// printf("len =%d ", len);
					ret += ft_putchar_ret('0');
				}
	...
	else if (!option->pad_deux)  // 5.2
	{
		if (option->pad_un) // 5.2.1
		{
			if (option->left_zeros) // 5.2.1.1
			{
				// printf("i = %i || len = %d\n", i, len);
				// while (++i < option->pad_un - len + (nb_have_a_sign(option, nb) && !option->left_justify) - (option->space))
				while (++i < option->pad_un - len - nb_have_a_sign(option, nb)
				- (option->space && !nb_have_a_sign(option, nb)
				&& option->pad_un > option->pad_deux ? 1 : 0))
				{
					// printf("len =%d ", len);
...
int		print_zeros_padding_after(t_options *option, int len, long long nb) // sert a print les 0 padding en toute circonstances
{
	...
	if (option->pad_deux) // 6.1
	{
		if (option->pad_un) // 6.1.1
		{
	...
	else if (!option->pad_deux)  // 6.2
	{
		if (option->pad_un) // 6.2.1
		{
...
/* int   before;
	...
	// printf("\n%d.%d.%d.%d.%d\n", after ? 2 : 1, option->left_justify ? 1 : 2, option->pad_un ? 1 : 2, option->pad_deux ? 1 : 2, option->left_zeros ? 1 : 2);
	...
	if (before) // 1.before
	{
		...
		{
			if (option->pad_un) // 1.1.1
			{
				if (option->pad_deux) // 1.1.1.1
				{
				...
				else if (!option->pad_deux) // 1.1.1.2
				{
			...
			else if (!option->pad_un) // 1.1.2
			{
				if (option->pad_deux) // 1.1.2.1
				{
				...
				else if (!option->pad_deux) // 1.1.2.2
				{
		...
		{
			if (option->pad_un) // 1.2.1
			{
				if (option->pad_deux) // final 1.2.1.1
				{
				...
				else if (!option->pad_deux) // 1.2.1.2
				{
			...
			else if (!option->pad_un) // 1.2.2
			{
				if (option->pad_deux) // final 1.2.2.1
				{
				...
				else if (!option->pad_deux) // 1.2.2.2
				{
	...
	else if (after) // 2.after
	{
		if (option->left_justify) // 2.1
		{
			...
			{
				if (option->pad_un) // 2.1.1.1
				{
					if (option->pad_deux) // final 2.1.1.1.1
					{
						// write(1, "h", 1);
						ret += print_spaces_padding_after(option, len, nb, after);
					}
					else if (!option->pad_deux) // final 2.1.1.1.2
					{
						// write(1, "h", 1);
						ret += print_zeros_padding_after(option, len, nb);
					}
				}
				else if (!option->pad_un) // 2.1.1.2
				{
					if (option->pad_deux) // final 2.1.1.2.1
					{
						// write(1, "h", 1);
						ret += print_zeros_padding_after(option, len, nb);
					}
					else if (!option->pad_deux) // final 2.1.1.2.2
					{
						// write(1, "h", 1);
						ret += print_zeros_padding_after(option, len, nb);
					}
			...
			{
				if (option->pad_un) // 2.1.2.1
				{
					if (option->pad_deux) // final 2.1.2.1.1
					{
						// write(1, "h", 1);
						ret += print_spaces_padding_after(option, len, nb, after);
					}
					else if (!option->pad_deux) // final 2.1.2.1.2
					{
						// write(1, "h", 1);
						ret += print_spaces_padding_after(option, len, nb, after);
					}
				}
				else if (!option->pad_un) // 2.1.2.2
				{
					if (option->pad_deux) // final 2.1.2.2.1
					{
						// write(1, "h", 1);
						ret += print_spaces_padding_after(option, len, nb, after);
					}
					else if (!option->pad_deux) // final 2.1.2.2.2
					{
						// write(1, "h", 1);
						ret += print_spaces_padding_after(option, len, nb, after);
					}
