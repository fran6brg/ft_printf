...
int		get_sign(t_options *option)
{
	...
	{
		if ((option->neg && !option->sign_is_print) || (!option->neg && option->sign)) // ok
			ret = 1;
	}
	return (ret);
}
...
int		print_spaces_padding_before(t_options *option, int len, int nb, int after) // sert a print les spaces padding en toute circonstances
{
	...
	// printf("(2) len = %d || get_sign(option) = %i || un_space_a_til_ete_print = %i\n", len, get_sign(option), un_space_a_til_ete_print);
	sign_is_print = 0;
	get_sign_option = get_sign(option);
	...
	while (++i < option->pad_min - len - (get_sign_option && !option->neg) - un_space_a_til_ete_print)
	{
		// printf("get_sign_option = %i ||", get_sign_option);
		if (option->pad_min && i + 1 == (option->pad_min - len - (get_sign_option && !option->neg) - un_space_a_til_ete_print))
		{
...
int		print_zeros_padding_before(t_options *option, int len, int nb) // sert a print les 0 padding en toute circonstances
{
	...
	if (option->pad_max) // 5.1
	{
		if (option->pad_min) // 5.1.1
		{
			...
			{
				if (option->pad_max >= option->pad_min)
				{
					//printf("len =%d || get_sign(option) = %i || !option->left_justify = %i", len, get_sign(option), !option->left_justify);
					while (++i < option->pad_max - len + option->neg + (get_sign(option) && !option->left_justify) /* le signe apparaît avant si left_justify */)
					{
						ret += ft_putchar_ret('0');
					}
				}
				...
				{
					//printf("len =%d || get_sign(option) = %i || !option->left_justify = %i", len, get_sign(option), !option->left_justify);
					while (++i < option->pad_max - len + (get_sign(option) && !option->left_justify) /* le signe apparaît avant si left_justify */)
					{
						ret += ft_putchar_ret('0');
					}
				}
	...
	else if (!option->pad_max)  // 5.2
	{
		if (option->pad_min) // 5.2.1
		{
			...
			{
				// printf("i = %i || len = %d\n", i, len);
				while (++i < option->pad_min - len + (get_sign(option) && !option->left_justify) - (option->space))
				{
					// printf("len =%d ", len);
					ret += ft_putchar_ret('0');
				}
			...
			{
				// printf("len = %d || 2 = %i", len, (get_sign(option) && !option->left_justify));
				while (++i < option->pad_min - len + (get_sign(option) && !option->left_justify) - (option->space))
				{
					// printf("len =%d ", len);
					ret += ft_putchar_ret('0');
